...
func (oic *OrderItemCreate) SetPricePerUnit(i int) *OrderItemCreate {
	oic.mutation.SetPricePerUnit(i)
	return oic
}
...
func (oic *OrderItemCreate) SetProduct(s string) *OrderItemCreate {
	oic.mutation.SetProduct(s)
	return oic
}

// SetID sets the "id" field.
func (oic *OrderItemCreate) SetID(i int) *OrderItemCreate {
...
func (oic *OrderItemCreate) AddDeliveryIDs(ids ...int) *OrderItemCreate {
	oic.mutation.AddDeliveryIDs(ids...)
	return oic
}

// AddDeliveries adds the "deliveries" edges to the Delivery entity.
func (oic *OrderItemCreate) AddDeliveries(d ...*Delivery) *OrderItemCreate {
	ids := make([]int, len(d))
	for i := range d {
		ids[i] = d[i].ID
	}
	return oic.AddDeliveryIDs(ids...)
}
...
func (oic *OrderItemCreate) check() error {
	...
	if _, ok := oic.mutation.Product(); !ok {
...
func (oic *OrderItemCreate) createSpec() (*OrderItem, *sqlgraph.CreateSpec) {
	...
	if value, ok := oic.mutation.Product(); ok {
	...
	if nodes := oic.mutation.DeliveriesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
